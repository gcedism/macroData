...
class Database():
    ...
    def add_items(self, new_var: dict):
        ...
        for source in SOURCES:
            _items = [self.var[x]['source']['kwargs'] for x in self.var
    ...
    def _aux_blsData(self, url, p, h, kwargs: dict):
        ...
        b = {
            k: pd.DataFrame(v, columns=['value']).rename(columns={'value': mapping[k]})
    ...
    def _get_census(self, kwargs: dict):
        ...
        for tic in kwargs:
            ...
            _data[tic['name']] = (pd.read_csv(io.StringIO(_s.decode('utf-8')), **p_csv)
                                  .iloc[:-2].loc[:, 'Value'].values)
    ...
    def _get_inegi(self, kwargs):
        ...
        for x in kwargs:
            ...
            + x['id'] + '/en/0700/false/' + x['fuente'] + '/2.0/' + API_KEYS['INEGI_API_KEY']
            s = requests.get(url, p).text
