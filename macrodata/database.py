...
class Database():
  ...
  def add_items(self, new_var: dict):
    ...
    for source in SOURCES:
      _items = [
        self.var[x]['source']['kwargs'] for x in self.var
        if self.var[x]['source']['name'] == source
      ]
      if _items != []:
        new_data = getattr(self, '_get_' + source)(_items)
        self.data = self.data.join(new_data, how='outer')

    self.data = (self.data.replace(',', '', regex=True).replace(
      ' - ', np.nan,
      regex=True).astype(float).interpolate(limit_area='inside'))
  ...
  def _aux_blsData(self, url, p, h, kwargs: dict):
    ...
    b = {
      k: pd.DataFrame(v, columns=['value']).rename(columns={
        'value': mapping[k]
      }).sort_index(ascending=False).set_index(pd.Index(range(len(v))))
      for k, v in a.items()
    }
  ...
  def _get_fred(self, kwargs: dict):
    ...
    _data.index = _data.index.map(
      lambda x: (eDate(dt.strptime(x, '%Y-%m-%d').date(), 1) - CDay(1)).date())
  ...
  def _get_census(self, kwargs: dict):
    ...
    p_csv = {'skiprows': [0, 1, 2, 3, 4, 5, 6], 'index_col': 0}
    ...
    for tic in kwargs:
      ...
      _data[tic['name']] = pd.read_csv(io.StringIO(_s.decode('utf-8')),
                                       **p_csv).iloc[:-2].loc[:,
                                                              'Value'].values
  ...
  def _get_fed(self, kwargs):
    ...
    _data = {}
    for tic in kwargs:
      p_csv = {'index_col': 0, 'skiprows': 5, 'usecols': tic['usecols']}
  ...
  def _get_philly(self, kwargs):
    ...
    _df.index = _df.index.map(
      lambda x: EoXMonth(dt.strptime(x, '%b-%y').date(), 0))
    ...
    for tic in kwargs:
      ...
      try:
        firstRow = [
          ...
          if str(df.index[x]).lower() == '1993 jan'
        ][0]
      except:
        firstRow = 0
      lastRow = [
        x for x in range(df.shape[0]) if pd.isna(df.index[x])
      ][0] if len([x for x in range(df.shape[0])
                   if pd.isna(df.index[x])]) > 0 else df.shape[0]
      df = df.iloc[firstRow:lastRow]
      df.index = [
        EoXMonth(dt.strptime(x, '%Y %b').date(), 0) for x in df.index
      ]
  ...
  def _get_ec(self, kwargs: dict):
    ...
    for tic in kwargs:
      ...
      _data = df.pivot_table(values='OBS_VALUE',
                             index='TIME_PERIOD',
                             columns='geo',
                             aggfunc=np.sum)
      _data.columns = [tic['name']]
      _data.index = _data.index.map(
        lambda x: EoXMonth(dt.strptime(x, '%Y-%m').date(), 0))
      data.append(_data)
  ...
  def _get_can(self, kwargs):
    ...
    p_csv = {'skiprows': 23, 'usecols': [0, 1, 3, 5], 'index_col': 0}
  ...
  def _get_bcb(self, kwargs):
    ...
    a = client.service.getValoresSeriesVO(in0=series,
                                          in1=DATES['start_2'],
                                          in2=DATES['end_2'])
    dates = [
      EoXMonth(
        dt(a[0]['valores'][j]['ano'], a[0]['valores'][j]['mes'],
           a[0]['valores'][j]['dia']).date(), 0)
      for j in range(len(a[0]['valores']))
    ]

    _data = pd.DataFrame(
      [[a[i]['valores'][j]['svalor']['_value_1'] for j in range(len(dates))]
  ...
  def _get_inegi(self, kwargs):
    ...
    for x in kwargs:
      url = 'http://en.www.inegi.org.mx/app/api/indicadores/desarrolladores/jsonxml/INDICATOR/' + x[
        'id'] + '/en/0700/false/' + x['fuente'] + '/2.0/' + API_KEYS[
          'INEGI_API_KEY']
      s = requests.get(url, p).text
      _d = pd.DataFrame(
        json.loads(s)['Series'][0]['OBSERVATIONS']).set_index('TIME_PERIOD')
      _d.index = _d.index.map(
        lambda x: EoXMonth(dt.strptime(x, '%Y/%m').date(), 0))
